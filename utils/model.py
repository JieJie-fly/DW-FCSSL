...
class Encoder(nn.Module):
    ...
    def forward(self, x1, x2):  # x: B x T x input_dims     B是batch长度；T是时间长度；
        x1.to(device)
        x2.to(device)
        if self.training:
            x_whole = self.input_fc(x1.float())
            x_whole = x_whole.transpose(1, 2)
            x_whole = self.feature_extractor(x_whole)  # B x Ch x T  20*2*27
        ...
        if self.training:
            # interphead(x_interp)是将提取的特征进行重构，Decoder
